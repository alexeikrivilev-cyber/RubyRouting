...
class CausalCrashBlockingProvider
  def initialize(gate)
    @gate = gate
  end
  ...
  def initiate(request)
    ...
    STDOUT.flush
    @gate.pop
    raise "causal crash probe must terminate before provider completion"
  end

  def resolve(_request)
    raise "causal crash probe must not resolve before process death"
  end
end

path, payout_id = ARGV
abort "usage: fresh_process_causal_dispatch.rb JOURNAL_PATH PAYOUT_ID" unless path && payout_id
...
gate = Queue.new
provider = CausalCrashBlockingProvider.new(gate)
...
Thread.new do
  begin
    ...
    warn "causal crash probe invocation error: #{error.class}: #{error.message}"
  end
end
