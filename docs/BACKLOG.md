...
- Post-cleanup baseline is clean on `main` at `1f3500e...`; the v0.3 convergence revision is published on `main` at `000931b...` with a clean working tree.
- `State::FactStore` now owns append-only fact identity/revision/journal delivery behind the coordinator atomic facade; restart continuation is covered by crash, corruption and long-history campaigns, and exact-revision closure passed.
...
- D-206 follow-up: the pre-publish matrix was test 455/9,219 (seed 53445), property 4/1,210 (seed 51414), model 3/2,941 (seed 33545), concurrency 12/946 (seed 14725) and fault 240/3,352 (seed 56648), all with zero failures/errors/skips. A final pre-commit rerun was test 455/9,220 plus property 4/1,210, model 3/2,941, concurrency 12/946 and fault 240/3,352, again with zero failures/errors/skips. Degradation completed in 4.4953 s; benchmark/load/replay metrics are bounded figures above. Exact-revision GitHub Actions run `33333118660` passed both jobs on `000931b...`; the fresh A–L closure is recorded in the active ExecPlan and v0.3 is `VERSION_COMPLETE`.
