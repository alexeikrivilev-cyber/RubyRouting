...
Status: VERSION_COMPLETE / HISTORICAL BASELINE

Superseded as active pre-TZ authority by `specifications/008-pre-tz-adversarial-edge-hardening.md`. This specification remains a protected regression contract and rationale for v0.3.3.
...
Before `VERSION_COMPLETE` the agent must enter `VERSION_CANDIDATE` and perform a fresh discovery pass from actual code without using backlog completion as a premise. At minimum challenge configuration/decision concurrency, malformed input, mixed-age/mixed-currency/sparse evidence, restart with changed clock origin/config, policy overlap, recovery timing, live/restore/replay equivalence, query/API behavior and history scaling.

Any newly found locally solvable P0/P1 reopens development. A closure claim requires a fresh exact-HEAD verification matrix, current CI, documentation consistency and explicit adversarial-discovery evidence.

## Historical closure

v0.3.3 passed this gate on product revision `f59bdd60c268a3982e32ad45353bf4c292f14153`; the status/documentation closure commit `b490c122d9e7cafb7f4beb60457c9060a43512d5` also passed GitHub Actions. The completed v0.3.3 backlog is archived at `docs/PRE_TZ_BACKLOG_V03_3.md` and the completed ExecPlan at `docs/exec-plans/completed/pre-tz-skeptical-hardening.md`.

## Stop conditions (historical)

v0.3.3 stopped only after the skeptical closure gate passed on exact HEAD. Current work is governed by SPEC-008 unless the authoritative TZ arrives.